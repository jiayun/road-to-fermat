...
本專案使用生成式 AI 產生內容，作為數學學習與探索用的手稿。內容可能包含數學錯誤、證明缺漏或不精確的引用，請配合原始文獻核對，歡迎透過 Issues 或 Pull Requests 提出勘誤。
...
- **章稿與說明文件**採用 [Creative Commons 姓名標示 4.0 國際授權（CC BY 4.0）](LICENSE-CC-BY-4.0.txt)：允許分享、翻譯、修改及商用，須依條款標示來源、附授權連結並註明修改。
- **程式碼、編譯工具與設定**採用 [MIT License](LICENSE-MIT)：允許使用、修改及商用，須保留版權與授權聲明。

完整適用範圍與建議署名方式見 [LICENSE.md](LICENSE.md)。授權僅涵蓋專案貢獻者依法擁有且有權授予的權利；第三方內容依其原有權利與授權處理。對依法不受著作權保護的 AI 生成內容、數學事實或公共領域素材，本專案不藉由授權聲明主張額外權利。
